...
namespace charlie
{
    namespace network
    {
        ...
        IPAddress ip_string_to_ip_address(PCSTR p_address_string)
        {
            char buffer[sizeof(uint32)];
            uint8 ip_bits[4];
            if(!inet_pton(AF_INET, p_address_string, buffer))
                return IPAddress({});
            ...
            ip_bits[2] = buffer[2];
            ip_bits[3] = buffer[3];
            return IPAddress(ip_bits[0], ip_bits[1], ip_bits[2], ip_bits[3], 0);
        }


        bool IPAddress::get_bitmask(const IPAddress& p_address, IPAddress& out_bitmask)
        {
            if(p_address.host_ == 0)
                return false;
            ...
            out_buffer_len = sizeof(IP_ADAPTER_INFO);
            if(GetAdaptersInfo(adapter_info, &out_buffer_len) != ERROR_SUCCESS)
            {
                free(adapter_info);
                adapter_info = (IP_ADAPTER_INFO*)malloc(out_buffer_len);
            }

            if((error = GetAdaptersInfo(adapter_info, &out_buffer_len)) != ERROR_SUCCESS)
            {
            ...
            while(current)
            {
                address = ip_string_to_ip_address(current->IpAddressList.IpAddress.String);
                address.port_ = p_address.port_;

                if(address == p_address)
                {
                    to_return = true;
                    out_bitmask = ip_string_to_ip_address(current->IpAddressList.IpMask.String);
                }
        ...
        bool NetworkStreamWriter::serialize(const std::string& string)
        {
            if(!can_write_bytes(*this, string.size())) { return false; }
        ...
        bool NetworkStreamReader::serialize(std::string& string)
        {
            char* current = (char*)at_;
            while(*current != '\0')
            {
                string.push_back(*current);
                current++;

                if((uint8*)current == &data()[length()])
                {
                    string.clear();
        ...
        bool Connection::has_timedout(const Time& time) const
        {
           // todo: replace hard coded timeout limit
            return (time - last_received_time_) >= Time(8.0);
        }
		...
		ServerDiscoveryConnection::ServerDiscoveryConnection()
			: m_initialised(false)
			, m_send_rate(3.0)
			, m_last_time_sent(0.0)
			, m_counter(0)
			, m_port(0)
		{

		}
		bool ServerDiscoveryConnection::found_server()
		{
			return !m_discovered_addresses.empty();
		}
		bool ServerDiscoveryConnection::initialize(const IPAddress& address, uint16 p_port)
		{
			if(!m_socket.open(address))
			{
				int error_code = Error::get_last();
				return false;
			}
		...
		void ServerDiscoveryConnection::discover()
		{
			const Time time = Time::now();
			if((time - m_last_time_sent) >= m_send_rate)
			{
				m_last_time_sent = time;

				DynamicArray<IPAddress> addresses;
                IPAddress               bitmask;
                IPAddress               broadcast;
                
                broadcast.port_ = m_port;

				if(IPAddress::local_addresses(addresses))
				{
					for(auto& address : addresses)
					{
						NetworkStream           stream;
						NetworkStreamWriter     writer(stream);
						ProtocolRequestPacket   packet(0);
                        ...
                        if(IPAddress::get_bitmask(address, bitmask))
                        {
						    broadcast.host_ = address.host_ | ~bitmask.host_;

						    printf("Broadcast to: \t %s\n", broadcast.as_string());
						    send_stream(broadcast, stream);
                        }
					}
				}
			}
		...
		DynamicArray<IPAddress>& ServerDiscoveryConnection::get_addresses()
		{
			return m_discovered_addresses;
		}
		void ServerDiscoveryConnection::receive()
		{
			IPAddress address;
			NetworkStream stream;
			...
			{
				bool exists = false;
				for(auto& addr : m_discovered_addresses)
				{
					if(addr == address)
					{
						exists = true;
				...
				if(!exists)
				{
					for(auto& li : m_broadcast_listeners) li->on_broadcast(address);
					m_discovered_addresses.push_back(address);
				}
				stream.reset();
			}
		}
		void ServerDiscoveryConnection::send_stream(const IPAddress& address, const NetworkStream& stream)
		{
			...
			if(!m_socket.send(address, stream.buffer_, stream.length_))
			{
				const int32 error_code = Error::get_last();
				if(Error::is_critical(error_code))
				{
		...
		void ServerDiscoveryConnection::add_broadcast_listener(IBroadcastListener* listener)
		{
			m_broadcast_listeners.push_back(listener);
		}

		void ServerDiscoveryConnection::remove_broadcast_listener(IBroadcastListener* listener)
		{
			for(int i = 0; i < m_broadcast_listeners.size(); i++)
				if(m_broadcast_listeners[i] == listener)
					m_broadcast_listeners.erase(m_broadcast_listeners.begin() + i);
		}
